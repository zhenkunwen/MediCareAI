"""Patient messaging API — conversations with doctors."""
...
async def list_conversations(
    ...
    items = await get_conversations_for_user(db, ctx.user.id, "patient", skip, limit)
...
async def get_conversation_messages(
    ...
    try:
        ...
        messages = await get_messages(
            db, uuid.UUID(conversation_id), ctx.user.id, "patient",
...
async def read_conversation(
    ...
    if not ctx.user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    count = await mark_conversation_read(db, uuid.UUID(conversation_id), "patient")
...
async def get_unread_count(
    ...
    if not ctx.user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    items = await get_conversations_for_user(db, ctx.user.id, "patient", limit=200)
...
async def delete_message_endpoint(
    ...
    if not ctx.user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    try:
        await delete_message(db, uuid.UUID(message_id), ctx.user.id, "patient")
...
async def delete_conversation_endpoint(
    ...
    if not ctx.user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    try:
        await delete_conversation(db, uuid.UUID(conversation_id), ctx.user.id, "patient")
        return {"status": "deleted"}
    except ValueError as e:
        raise HTTPException(status_code=409, detail=str(e))
